...
-- Consultas
-- Punto 01
select enombre, cargo, salario,  comision from empleados where salario < 2000 and (comision = 0 or comision is NULL);

-- Punto 2.1
select * from empleados where fechaingreso like '___SEP-81';
-- Punto 2.2
select * from empleados where fechaingreso between to_date('01091981', 'ddmmyyyy') and to_date('30091981', 'ddmmyyyy');

--Punto 3
select enombre, dnombre, cargo, empid, fechaingreso, ciudad 
from empleados join departamentos 
on empleados.deptid = departamentos.deptid
order by enombre;

-- Punto 4
select emp.enombre as empnombre, jef.enombre as jefnombre
from empleados emp join empleados jef
on emp.empid = jef.jefe
where emp.jefe is not NULL;

-- Punto 5
select dnombre, count(*) as numempleados
from empleados
join departamentos
on empleados.deptid = departamentos.deptid
group by dnombre;
